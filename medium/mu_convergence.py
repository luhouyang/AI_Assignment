...
def printSchedule(schedule, log):
    write_str = ''
    ...
    if (MODE == 'mu_conv'):
        with open('mu_convergence_result.txt', 'a+') as f:
            f.write('\n' + write_str + f'\n;\n')
            f.close()

        with open('mu_convergence_medium.txt', 'a+') as f:
    ...
    log_str = ""
    for i in range(4000):
        log_str += f"{log.select('gen')[i]},{log.select('min')[i][0]},{log.select('min')[i][1]},{log.select('min')[i][2]},{MU_INDPB}\n"
    postfix = str(MU_INDPB)
    postfix = postfix.replace('.', '')

    with open(f'C:/Users/User/Desktop/Python/ai_assignment/medium/mu_convergence_logs/mu_c_{postfix}.txt', 'w+') as f:
        f.write(log_str)
        f.close()
